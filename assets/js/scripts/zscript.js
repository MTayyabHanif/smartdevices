...
// scrollto function - useful
function scrollToElement(elementId) {
	var top = jQuery(elementId).offset().top;
	jQuery('html,body').animate({ scrollTop: top-80 }, 850);
}
...
jQuery( function( $ ) {
    ...
    // show specs
    .on( 'click', '#show-specs', function(e) {
        e.preventDefault();
        $( '.specifications_tab a' ).click();
        scrollToElement('.wc-tabs-wrapper');
        return true;
    } )
    // Review link
...
jQuery(document).ready(function ($) {
    ...
    //filter button click
    $('.woocommerce-filter>span.button').click(function() {
        scrollToElement('#secondary');
        return true;
    });
	...
	*/
	...
	//search bar
	$('.desktop#header-search>span').click(function() {
        $(this).find('i').toggleClass('ti-search ti-close');
		$('.search-form-wrapper').toggleClass('active');
		$('.search-field').focus();
		$('.searchbar-backdrop').fadeToggle();
	});

    //search bar backdrop
    $('.searchbar-backdrop').click(function() {
        var $window = $(window);
        var windowsize = $window.width();
        if (windowsize < 520 || windowsize == 520) {
            $('.mobile#header-search>span').find('i').toggleClass('ti-search ti-close');
            $('.mobile-search').fadeToggle();
            $('.searchbar-backdrop').fadeToggle();
        }else{
            $('#header-search>span').find('i').toggleClass('ti-search ti-close');
            $('.search-form-wrapper').toggleClass('active');
            $('.searchbar-backdrop').fadeToggle();
        }
    });
	...
	//mobile search bar
	$('.mobile#header-search>span').click(function() {
        //closing menu is opened
        if ($('.menu-wrapper').hasClass('is-open')) {
            $('.menu-wrapper').removeClass('is-open');
            $('a.toggle-menu').find('span').removeClass('ti-close').addClass('ti-menu');
        }
	...
	*/
	...
	Promise.all([baseFont.load(null, 18000), baseFont2.load(null, 18000), baseFont3.load(null, 18000), baseFont4.load(null, 18000), baseFont5.load(null, 18000)]).then(function () {

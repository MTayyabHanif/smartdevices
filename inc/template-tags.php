...
if ( ! function_exists( 'smartdevices_posted_on' ) ) :
	/**
	 * Prints HTML with meta information for the current post-date/time and author.
	 */
	function smartdevices_posted_on() {
		...
		$posted_on = sprintf(
			...
			esc_html_x( 'Posted on %s', 'post date', 'smartdevices' ),
	...
	function smartdevices_entry_footer() {
		...
		if ( 'post' === get_post_type() ) {
			...
			if ( $categories_list ) {
				...
				printf( '<span class="cat-links">' . esc_html__( 'Posted in %1$s', 'smartdevices' ) . '</span>', $categories_list ); // WPCS: XSS OK.
			}
			...
			if ( $tags_list ) {
				...
				printf( '<span class="tags-links">' . esc_html__( 'Tagged %1$s', 'smartdevices' ) . '</span>', $tags_list ); // WPCS: XSS OK.
			}
		}
